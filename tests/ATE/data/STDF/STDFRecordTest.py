...
class STDFRecordTest:
    ...
    def assert_var_bits(self, expected_value):
        ...
        if self.debug:
            print(
                f"found bits length {bits_count} | expected_value {len(expected_value)}"
            )
        ...
        for byte_index in range(byte_length):
            ...
            for bit_indx in range(8):
                if (bit_indx + byte_index * 8) < bits_count:
                    if self.debug:
                        print(
                            f"found_value {bits[7-bit_indx]} | expected_value {expected_value[bit_indx + byte_index*8]}"
                        )
                    assert (
                        bits[7 - bit_indx] == expected_value[bit_indx + byte_index * 8]
                    )
    ...
    def assert_instance_field(self, instance, element_index, expected_value):
    ...
    def assert_instance_record_header(self, instance, rec_len, rec_type, rec_sub):
