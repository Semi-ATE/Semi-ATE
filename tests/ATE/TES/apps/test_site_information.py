...
PRR_RECORD = {'type': 'PRR', 'REC_LEN': None, 'REC_TYP': 5, 'REC_SUB': 20,
...
def site_info_handler():
    site_info = SiteInformationHandler([SITE_NUM])

    return site_info


def test_set_sites_information(site_info_handler: SiteInformationHandler):
    site_info_handler.set_sites_information(BIN_TABLE)
    assert site_info_handler.get_num_sites() == 1


def test_no_test_result_stored(site_info_handler: SiteInformationHandler):
    assert len(site_info_handler.get_summary_information()) == 0


def test_store_test_result(site_info_handler: SiteInformationHandler):
    site_info_handler.set_sites_information(BIN_TABLE)
    site_info_handler.store_test_result(SITE_NUM, PRR_RECORD)
