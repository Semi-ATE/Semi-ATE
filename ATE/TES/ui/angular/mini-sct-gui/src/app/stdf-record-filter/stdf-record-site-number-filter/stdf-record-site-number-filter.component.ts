...
export class StdfRecordSiteNumberFilterComponent implements OnInit, OnDestroy {
  ...
  siteNumberFilterCheckboxChanged() {
    let selectedSiteNumbers = this.computeSitesOfInterest(this.siteNumberInputConfig.value, 0, this.maxSiteNumber);
    this.siteNumberInputConfig.disabled = !this.siteNumberCheckboxConfig.checked;
    let isSubset = false;
    if (selectedSiteNumbers) {
      isSubset = this.isSubsetOfSelectedSiteNumbers(selectedSiteNumbers);
      this.selectedSiteNumbers = selectedSiteNumbers;
    }
    this.updateFilterAndPublish(isSubset);
  }

  siteNumberFilterValueChanged(inputValue: string ): void {
    let selectedSiteNumbers = this.computeSitesOfInterest(inputValue, 0, this.maxSiteNumber);
    if (selectedSiteNumbers) {
      let isSubset = this.isSubsetOfSelectedSiteNumbers(selectedSiteNumbers);
      this.selectedSiteNumbers = selectedSiteNumbers;
      this.siteNumberInputConfig.textColor = 'blue';
    ...
    this.storage.get(this.getStorageKey())
      .subscribe( e => {
        ...
        if (siteFilterSetting && typeof siteFilterSetting.selectedSites === 'string' && typeof siteFilterSetting.enabled === 'boolean' ) {
          ...
          if (siteFilterSetting.selectedSites !== '') {
            this.siteNumberInputConfig.value = siteFilterSetting.selectedSites;
            this.siteNumberFilterValueChanged(siteFilterSetting.selectedSites);
          }
        }
      });
  }
