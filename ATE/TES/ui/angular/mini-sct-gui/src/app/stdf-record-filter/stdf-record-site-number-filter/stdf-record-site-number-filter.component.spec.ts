  ...
  describe('Site Filter', () => {
    ...
    describe('siteNumberFilterCheckboxChanged', () => {
      it('should call updateFilterAndPublish', () => {
        ...
        component.siteNumberFilterCheckboxChanged();
    ...
    it('should only show stdf records of site 0 if site filter is active and configured to see only site 0', () => {
      ...
      component.siteNumberFilterValueChanged('0');

      expect(filterService.filteredRecords.length).toEqual(1);
      expect(filterService.filteredRecords[0].type).toEqual(StdfRecordType.Ptr);
      expect(filterService.filteredRecords[0].values.length).toEqual(1);
    });

    describe('siteNumberFilterValueChanged', () => {
      it('should set an error text in case of ivalid input like "a,b"', () => {
        const INVALID_INPUT = 'a,b';
        component.siteNumberFilterValueChanged(INVALID_INPUT);
  ...
  describe('restoreSettings', () => {
    ...
    it('should apply settings from storage', async (done) => {
      ...
      storage.set((component as any).getStorageKey(), setting).subscribe( async () => {
        ...
        await expectWaitUntil(
          () => fixture.detectChanges(),
          () => component.siteNumberCheckboxConfig.checked === setting.enabled &&
            component.siteNumberInputConfig.value === setting.selectedSites,
