...
describe('StdfRecordTypeFilterComponent', () => {
  ...
  describe('PTR Filter', () => {
    ...
    it('should remove all PTRs from the array fileteredRecords of the StdfRecordFilterService', () => {
      let ptr: StdfRecord = {
      ...
      expect(filterService.filteredRecords.some(e => e.type === StdfRecordType.Ptr)).toBeFalse();
      component.recordTypeFilterChanged(true, StdfRecordType.Ptr);
      expect(filterService.filteredRecords.some(e => e.type === StdfRecordType.Ptr)).toBeTrue();
    });
  });
