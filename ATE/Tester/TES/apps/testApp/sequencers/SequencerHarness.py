...
from ATE.Tester.TES.apps.testApp.sequencers.ExecutionPolicy import get_execution_policy, ExecutionType
...
class SequencerHarness(Harness.Harness):
    _statemachine: Optional[TheTestAppMachine]
    _mqtt: Optional[MqttClient]

    def __init__(self, sequencer: SequencerBase, params: dict, execution_strategy: StagesSequenceGenerator):
    ...
    def _generate_mqtt_connection(self, topic_factory: TopicFactory, submit_callback: callable):
        ...
        mqtt.on_command = self._execute_command
...
def main():
    return SequencerHarness.run_from_command_line(sys.argv)
